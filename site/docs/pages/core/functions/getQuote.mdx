...
`getQuote` owns an `AbortController` for proxy requests. It aborts the remaining stream
when selection returns a quote, returns `null`, or throws. This includes `fastest`,
`firstN`, benchmark, and custom selection strategies.

Aborting a proxy stream does not guarantee cancellation of server-side provider or RPC work.
For direct preparation and selection, [use your own controller](./selectQuote.mdx).
