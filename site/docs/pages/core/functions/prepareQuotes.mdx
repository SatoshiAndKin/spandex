...
Optional signal for the proxy request and stream decoder. Keep it separate from `swap`
and simulation options. A pre-aborted signal rejects preparation before it sends a request.
For local providers, this signal does not stop provider requests or simulations already in progress.

The caller owns the controller. After selection, call `abort()` in a `finally` block.
See the [selection example](./selectQuote.mdx).
