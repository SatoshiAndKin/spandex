...
## Cancel a client stream

`getQuote` stops its proxy fetch and stream after selection finishes or fails.

Direct callers can pass an `AbortSignal` to `prepareQuotes({ config, swap, mapFn, signal })`
or `prepareSimulatedQuotes({ config, swap, simulationOptions, signal })`.
For calls on `AggregatorProxy`, use `proxy.prepareQuotes(swap, signal)` or
`proxy.prepareSimulatedQuotes(swap, simulationOptions, signal)`.
The signal controls the client request. It is separate from serialized swap and simulation options.

Use the [controller ownership example](/core/functions/selectQuote) to release the stream
in `finally`. Aborting this stream does not guarantee that server-side provider or RPC work stops.
